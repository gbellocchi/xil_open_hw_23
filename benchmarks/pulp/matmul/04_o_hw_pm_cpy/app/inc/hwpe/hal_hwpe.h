...
static inline unsigned int hwpe_get_finished() {
  return HWPE_READ(HWPE_FINISHED);
}

static inline unsigned int hwpe_get_register(unsigned int offset) {
  return HWPE_READ(offset);
}
...
static inline void hwpe_in1_addr_set(unsigned int value) {
  HWPE_WRITE(value, HWPE_A_ADDR);
}

static inline void hwpe_in2_addr_set(unsigned int value) {
  HWPE_WRITE(value, HWPE_B_ADDR);
}

static inline void hwpe_out_addr_set(unsigned int value) {
