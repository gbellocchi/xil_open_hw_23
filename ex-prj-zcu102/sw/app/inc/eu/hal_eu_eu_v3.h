...
#define eu_write32(add, val_) (*(volatile unsigned int *)(long)(add) = val_)

static inline uint32_t eu_read32(uint32_t add)
{
...
{
  return eu_read32(ARCHI_EU_DEMUX_ADDR + EU_CORE_BUFFER);
}
...
{
  return eu_read32(ARCHI_EU_DEMUX_ADDR + EU_CORE_BUFFER_MASKED);
}
...
{
  eu_write32(ARCHI_EU_DEMUX_ADDR + EU_CORE_BUFFER_CLEAR, evtMask);
}
...
{
  eu_write32(ARCHI_EU_DEMUX_ADDR + EU_CORE_MASK, evtMask);
}
...
#else
  eu_write32(base + EU_CORE_MASK_OR, evtMask);
#endif
}
...
#else
  eu_write32(ARCHI_EU_DEMUX_ADDR + EU_CORE_MASK_AND, evtMask);
#endif
}
...
{
  return eu_read32(ARCHI_EU_DEMUX_ADDR + EU_CORE_EVENT_WAIT);
}
...
{
  return eu_read32(ARCHI_EU_DEMUX_ADDR + EU_CORE_BUFFER_IRQ_MASKED);
}

static inline void eu_irq_mask(unsigned int irqMask)
{
  eu_write32(ARCHI_EU_DEMUX_ADDR + EU_CORE_MASK_IRQ, irqMask);
}

static inline void eu_irq_secMask(unsigned int irqMask)
{
  eu_write32(ARCHI_EU_DEMUX_ADDR + EU_CORE_MASK_SEC_IRQ, irqMask);
}

static inline void eu_irq_maskSet_base(unsigned int base, unsigned int irqMask)
{
  eu_write32(base + EU_CORE_MASK_IRQ_OR, irqMask);
}
...
{
  eu_write32(ARCHI_EU_DEMUX_ADDR + EU_CORE_MASK_IRQ_AND, irqMask);
}

static inline void eu_irq_secMaskSet(unsigned int irqMask)
{
  eu_write32(ARCHI_EU_DEMUX_ADDR + EU_SEC_DEMUX_OFFSET + EU_SEC_MASK_OR, irqMask);
}

static inline void eu_irq_secMaskClr(unsigned int irqMask)
{
  eu_write32(ARCHI_EU_DEMUX_ADDR + EU_SEC_DEMUX_OFFSET + EU_SEC_MASK_AND, irqMask);
}
...
{
  eu_write32(barAddr + EU_HW_BARR_TRIGGER_MASK, coreMask);
  eu_write32(barAddr + EU_HW_BARR_TARGET_MASK, targetMask);
}
...
{
  eu_write32(barAddr + EU_HW_BARR_TRIGGER, mask);
}
...
{
  return eu_read32(ARCHI_CLUSTER_PERIPHERALS_ADDR + ARCHI_EU_OFFSET + EU_SOC_EVENTS_AREA_OFFSET + EU_SOC_EVENTS_CURRENT_EVENT);
}

#ifdef ARCHI_HAS_FC
static inline unsigned int fc_eu_soc_events_pop()
{
  return eu_read32(ARCHI_FC_ADDR + ARCHI_EU_OFFSET + EU_SOC_EVENTS_AREA_OFFSET + EU_SOC_EVENTS_CURRENT_EVENT);
}
#endif
...
{
  eu_write32(barAddr + EU_SOC_HW_BARR_TRIGGER, clusterMask);
}

static inline void eu_soc_bar_setup(unsigned int barAddr, unsigned int clusterMask)
{
  eu_write32(barAddr + EU_SOC_HW_BARR_TRIGGER_MASK, clusterMask);
}
...
static inline void fc_eu_trig(unsigned int event) {
  eu_write32(0x50000000 + PULP_CKGATE_OFFSET + 0x20C, 1 << event);
}
#endif
